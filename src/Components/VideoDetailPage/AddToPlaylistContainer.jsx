		...
		<>
			...
			{state.playlists.map((playlist) => (
				<button
					...
					<i
						className={
							isAlreadyAdded(playlist.videoList, video._id)
								? 'fas fa-check-circle btn-icon'
								: 'fas fa-plus-circle btn-icon'
						}></i>
					{playlist.title}
			...
			<button
				style={{ display: openAddPlaylistInput ? 'none' : 'block' }}
				type='button'
				className='btn btn-text-icon-secondary'
				onClick={() => setAddPlaylistInput(true)}>
				<i className='fas fa-plus-circle btn-icon'></i>Add new playlist
			</button>
			...
			{openAddPlaylistInput && (
				<div className='add-new-playlist-input'>
					<form onSubmit={submitFormNewPlaylist}>
						...
						<button type='submit' className='btn btn-text-icon-secondary'>
							<i className='fas fa-plus-circle btn-icon primary-text-color'></i>
							Add new playlist
						</button>
					</form>
				</div>
