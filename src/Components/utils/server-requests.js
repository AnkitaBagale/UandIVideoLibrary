...
export const getNotesForVideo = async ({ userId, videoId, setNotes }) => {
	try {
		const {
			...
			`https://uandistoreapi.herokuapp.com/users/${userId}/notes/${videoId}`,
		);
...
export const CreateNoteForVideo = async ({
	...
	try {
		...
		const {
			data: { response },
		} = await axios.post(`https://uandistoreapi.herokuapp.com/notes`, newVideo);
...
export const UpdateNoteForVideo = async ({
	...
	try {
		...
		const {
			...
			`https://uandistoreapi.herokuapp.com/notes/${noteId}`,
			noteUpdates,
		);
